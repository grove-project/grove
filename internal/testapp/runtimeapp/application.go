...
func runtimeConfiguration(configuration testapp.Configuration, payload, canonical []byte) groveruntime.Configuration {
...
func groveShopStatus(status groveruntime.ClusterStatus) testapp.ClusterStatusView {
...
func groveShopArtifactStatus(status *groveruntime.ArtifactStatus) *testapp.ArtifactStatusView {
	if status == nil {
		return nil
	}
...
func runtimeInvalidConfiguration(source []byte) (groveruntime.Configuration, string, string, error) {
	...
	if validationErr == nil || !errors.As(validationErr, &validation) {
		return groveruntime.Configuration{}, "", "", errors.New("candidate configuration must violate Grove test app validation")
	}
