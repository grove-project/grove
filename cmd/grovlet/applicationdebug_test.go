...
func waitForDebugApplicationStates(ctx context.Context, statePath string, want map[grove.ServiceID]string) (groveshop.ClusterStatusView, error) {
	ticker := time.NewTicker(applicationConditionInterval)
	defer ticker.Stop()
...
func applicationComponentState(status groveshop.ClusterStatusView, serviceID grove.ServiceID) string {
