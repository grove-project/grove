...
func (c *applicationController) prepareConfiguredApplicationStartup(
	...
	if exists {
		...
		if record.ArtifactDigest == inspection.ArtifactDigest {
			...
			c.lastEvent = "matching Grove Shop cluster discovered; Join is the default action"
		} else {
			c.lastEvent = "Grove Shop cluster discovered with a different artifact; rollout is required"
		}
		c.mu.Unlock()
		return nil
	}
	...
	c.lastEvent = "No Grove Shop cluster discovered"
	c.mu.Unlock()
	return nil
}
...
func (c *applicationController) startDiscoveredApplicationCluster(ctx context.Context, args []string) (any, error) {
	...
	if discovery == nil || inspection.Config == nil || !startAvailable {
		return nil, errors.New("a new Grove Shop cluster is not available to start")
	}
	if _, exists, err := discovery.discover(ctx); err != nil {
		return nil, err
	} else if exists {
		return nil, errors.New("a compatible Grove Shop cluster was discovered; start is no longer available")
	}
	...
	node, ready, err := c.startDiscoveredApplicationNode(ctx, "node-1", "", webAddress, 1)
	if err != nil {
		return nil, fmt.Errorf("bootstrap one-node Grove Shop cluster: %w", err)
	}
	...
	c.lastEvent = "bootstrapped Grove Shop cluster with node-1"
	c.mu.Unlock()
...
func (c *applicationController) joinApplicationCluster(ctx context.Context, args []string) (any, error) {
	...
	node, ready, err := c.startDiscoveredApplicationNode(ctx, nodeID, seed.RouteURL, record.WebAddress, nodeNumber)
	if err != nil {
		return nil, fmt.Errorf("join Grove Shop cluster as %s: %w", nodeID, err)
	}
	record.NextNode++
	record.Revision++
	...
	c.startupNodes = 0
	c.lastEvent = nodeID + " joined the Grove Shop cluster"
	c.mu.Unlock()
...
func (c *applicationController) startDiscoveredApplicationNode(
	...
	case 2:
		args = append(args, "--grove-shop-inventory", "--grove-shop-shipping")
	case 3:
		args = append(args, "--grove-shop-payment")
	}
