...
func TestTargetCompilerDefaultsMatchBlankArtifactRuntime(t *testing.T) {
	...
	_, runtimeConfiguration, err := loadEmbeddedConfiguration()
	if err != nil {
		t.Fatal(err)
	}
	if compiled != runtimeConfiguration.Value.(groveshop.Configuration) {
...
func TestLoadEmbeddedGroveShopConfigurationUsesDefaults(t *testing.T) {
	...
	if want := groveshop.DefaultConfiguration(); configuration.Value != want {
