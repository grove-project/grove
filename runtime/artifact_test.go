...
func TestGroveShopArtifactDeploys(t *testing.T) {
	...
	cluster := startMembershipGrovlets(
		...
		[]string{
			...
			"--component", "orders",
			"--component", "web",
			"--component-listen", "web=" + webAddress,
		},
		[]string{
...
func assertGroveShopManifest(t *testing.T, inspection artifact.Inspection) {
	...
	if manifest.FormatVersion != artifact.FormatVersion || manifest.ApplicationID != "grove-test-app" || manifest.CodeVersion != "v0.1.0-dev" {
	...
	wantComponents := []artifact.Component{
		...
		{ServiceID: groveshop.ServicePayment, Name: "Payment", Runtime: "process", Entrypoint: []string{"worker", "--component", "payment"}},
		{ServiceID: groveshop.ServiceShipping, Name: "Shipping", Runtime: "process", Entrypoint: []string{"worker", "--component", "shipping"}},
...
func TestParseWebWorkerConfig(t *testing.T) {
	...
	if cfg.component != "web" || cfg.listenAddress != "127.0.0.1:8080" {
