...
var (
	...
	errConsoleUnavailable    = errors.New("Grove application console is not running")
)
...
func runApplicationConsole(ctx context.Context, args []string, input io.Reader, output io.Writer) error {
	...
	runtimeDir, err := os.MkdirTemp("", "grove-application-console-")
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("locate application artifact: %w", err)
	}
	controller := newApplicationController(executable, runtimeDir)
	defer controller.close()
	inspection, _, err := loadEmbeddedConfiguration()
	if err != nil {
		return fmt.Errorf("load application startup identity: %w", err)
	}
	if !inspection.ConfigEmpty {
		if err := controller.prepareConfiguredApplicationStartup(ctx, inspection); err != nil {
			return fmt.Errorf("prepare application startup: %w", err)
		}
	}
...
func resolveTUISelection(line string) (string, []string, bool) {
	...
	if len(parts) >= 2 {
		...
		case parts[0] == "Application" && parts[1] == "Run integrity check" && len(parts) == 2:
			return activeApplication.IntegrityAction, nil, activeApplication.IntegrityAction != ""
...
func renderApplicationTUI(ctx context.Context, tui *console.TUI, output io.Writer) error {
	...
	if err != nil {
		return fmt.Errorf("render application TUI: %w", err)
	}
	if _, err := io.WriteString(output, snapshot+"\nUse Section > Action > argument, or q Quit\n"); err != nil {
		return fmt.Errorf("write application TUI: %w", err)
	}
	return nil
}
